...
interface VideoSectionProps {
    title: string;
    content: string; // This can be either a YouTube URL or an embed code
}

export default function VideoSection({ title, content }: VideoSectionProps) {
    const [embedUrl, setEmbedUrl] = useState<string>('');
    
    useEffect(() => {
        // Check if content is a YouTube URL or an embed code
        if (content.includes('<iframe')) {
            // Extract src from iframe if it's an embed code
            const srcMatch = content.match(/src=["']([^"']+)["']/);
            if (srcMatch) {
                setEmbedUrl(srcMatch[1]);
            }
        } else {
            // Treat as a YouTube URL
            setEmbedUrl(getYouTubeEmbedUrl(content));
        }
    }, [content]);
    ...
    return (
        <section className="my-8 sm:my-12 md:my-16 flex flex-col items-center px-4 sm:px-6 md:px-8">
            <div className="border-2 sm:border-3 md:border-4 border-primary p-3 sm:p-4 md:p-6 rounded-md sm:rounded-lg w-full max-w-[1152px] bg-card shadow-md sm:shadow-lg">
                <div className="relative w-full rounded-md sm:rounded-lg md:rounded-xl overflow-hidden" style={{ paddingTop: '56.25%' }}>
                    <iframe
                        src={embedUrl}
